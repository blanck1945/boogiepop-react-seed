...
export const nodeDetails: Record<string, NodeDetail> = {
  host: {
    id: 'host',
    name: 'boogiepop-host',
    description: 'Shell central de la plataforma. Gestiona login, valida token vía SDK en cada navegación, y expone la sesión a los remotes via Module Federation (host-auth). El login es el único punto que emite JWT.',
    category: 'host',
    tech: ['React 19', 'Vite', 'Module Federation', 'React Router', 'boogiepop-auth-sdk'],
    relationships: [
      'POST /api/auth/login → backend — único punto que emite JWT',
      'resolveSessionFromJwt() — decodifica JWT localmente al cargar (sin red)',
      'useAuthGuard({ trigger: pathname }) — valida en cada cambio de ruta via /me',
      'onRefresh(me) — actualiza roles/workspaces con datos frescos del backend',
      'Expone host-auth (MF) → remotes leen sesión sin llamar /me de nuevo',
  ...
  'auth-sdk': {
    ...
    description: 'SDK centralizado de autenticación. Usado por el host Y los remotes — evita duplicar la lógica de /me. El flujo: JWT decode local en la carga, validación via /me en cada navegación, fetch wrapper para peticiones autenticadas.',
    category: 'lib',
    tech: ['TypeScript', 'npm package', 'React hooks'],
    relationships: [
      'resolveSessionFromJwt() — decodifica JWT sin red, retorna ANONYMOUS_SESSION si no hay token',
      'validateToken() — única función que llama GET /api/auth/me',
      'useAuthGuard({ trigger, onExpired, onRefresh }) — valida en cada cambio de ruta',
      'createAuthenticatedFetch() — fetch wrapper que agrega Bearer + maneja 401',
      'ANONYMOUS_SESSION — usuario base sin roles ni abilities (default cuando no hay sesión)',
      'useBoogiepopSession() — hook para remotes: lee host-bridge o token+me',
      'hasRole() / hasAbility() / hasWorkspace() — helpers de autorización',
    ],
...
export const initialEdges: Edge[] = [
  ...
  // Libs ← FR (npm imports)
  { ...edgeBase, id: 'h-as', source: 'host', target: 'auth-sdk', label: 'npm import', style: { stroke: '#a855f7', strokeWidth: 1.5 } },
